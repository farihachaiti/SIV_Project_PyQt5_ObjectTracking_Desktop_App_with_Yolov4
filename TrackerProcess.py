...
class MainWindow(QMainWindow):
    ...
    def start_process_two(self):
        if self.p is None:  # No process running.
            self.message("Executing process")
            VPlayer.playButton.setEnabled(False)
            VPlayer.stopButton.setEnabled(False)
            Plot.btn_plot.setEnabled(False)
            self.p = QProcess()  # Keep a reference to the QProcess (e.g. on self) while it's running.
            self.p.readyReadStandardOutput.connect(self.handle_stdout)
            self.p.readyReadStandardError.connect(self.handle_stderr)
            self.p.stateChanged.connect(self.handle_state)
            ...
            if os.path.exists(fileName):
                oldDir = os.path.basename(os.path.dirname(fileName))
                print(oldDir)
                shutil.rmtree("./outputs/" + oldDir)
                os.mkdir("./outputs/" + oldDir)
                print(oldDir)
            if self.path:
                self.command2 = "python object_tracker.py --video" + " " + self.path + " " + "--output"  + " " + fileName + " " + "--model yolov4 --dont_show --count --info"
            else:
